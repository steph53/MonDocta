...
const WhyUsPoint: React.FC<WhyUsPointProps> = ({
    ...
    return ( 
            <div ref={TextRef} className="flex flex-col justify-around lg:justify-normal lg:flex-row m-8 h-[90vh] opacity-0">
                <div className="flex flex-col lg:h-full justify-between sm:pt-4 gap-4 sm:gap-8 md:gap-10">
                    <h1 className="text-3xl sm:text-5xl lg:text-7xl block ">{number}</h1>
                    <h2 className="max-w-[90%] lg:pb-28 ml-2 text-xl sm:text-3xl md:text-5xl lg:text-8xl">{title}</h2>
                </div>
                <div className=" flex flex-col lg:max-w-[50%] justify-around lg:justify-between gap-10 lg:pb-28 lg:pt-16 h-[60vh] lg:h-full">
                    ...
                    <p className="text-lg sm:text-xl sm:pb-5 lg:text-3xl">{description}</p>
                </div>
            </div>
    );
}
